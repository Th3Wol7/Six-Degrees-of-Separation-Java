...
public class ActivityScreen extends JPanel implements ActionListener {
	...
	volunteer.setSelected(false);dance.setSelected(false);run.setSelected(false);shop.setSelected(false);read.setSelected(false);movie.setSelected(false);
	// suppose to clear text area
	area.setText(null);
	// OR area.setText(area.getText().replace(area.getSelectedText(),""));
	}if(e.getSource().equals(edit)){volunteer.setSelected(false);dance.setSelected(false);run.setSelected(false);shop.setSelected(false);read.setSelected(false);movie.setSelected(false);
	// for loop// 2. mayb can set a global string to collect all string then add it
	// to te textarea
	if(volunteer.isSelected()){
	String vol = volunteer.getText();area.setText(vol);}else if(dance.isSelected()){
	String dan = dance.getText();area.setText(dan);}else if(run.isSelected()){
	String runs = run.getText();area.setText(runs);}else if(shop.isSelected()){
	String shops = shop
			.getText();System.out.println("the value of shop is :"+shop);area.setText(shops);}else if(read.isSelected()){
	String reads = read.getText();area.setText(reads);}else if(movie.isSelected()){
	String movies = movie.getText();area.setText(movies);}}if(e.getSource().equals(add)){
	boolean isSelected = false;if(suggestionTable.getSelectedRow()!=-1){isSelected=true;
	// need to check if the value from the suggested table equals to any activity
	// the user have already if yes
	// suggest u add this activity already, joptionpane warning etc. else if not
	// then add the activity to the textbox
	...
	// mayb should pass it a list of string/ array then but that means a loop n a
	// incrementing varible to pass thru the array
	// compare(value, wat in textbox)

	}else{isSelected=false;JOptionPane.showMessageDialog(null,"There is no row that have been selected","Warning",JOptionPane.ERROR_MESSAGE);}}if(e.getSource().equals(remove)){
	boolean isSelected = false;if(suggestionTable.getSelectedRow()!=-1){isSelected=true;
	int choice = JOptionPane.showConfirmDialog(null, "Remove this Activity", "Remove prompt", JOptionPane.YES_NO_OPTION,
			JOptionPane.QUESTION_MESSAGE);if(choice==JOptionPane.YES_OPTION){suggestionTable.remove(suggestionTable.getSelectedRow());
}}}
